...
def epyt2pypes(inp_file, out_file, name):
    ...
    for n in G.getNodeIndex():
        ...
        if G.getNodeType(n).upper() == "JUNCTION":
            ...
            node_ids[n] = "Joint" + str(obj_counts["Joint"])
            nodes["Joint" + str(obj_counts["Joint"])] = node_obj
            obj_counts["Joint"] += 1

        elif G.getNodeType(n).upper() == "RESERVOIR":
            node_obj = {
                "id": "Reservoir" + str(obj_counts["Reservoir"]),
                "type": "Reservoir",
                "contents": content_placeholder,
                "levation (meters)": G.getNodeElevations(n),
                "tags": {},
            }
            node_ids[n] = "Reservoir" + str(obj_counts["Reservoir"])
            nodes["Reservoir" + str(obj_counts["Reservoir"])] = node_obj
            obj_counts["Reservoir"] += 1

        elif G.getNodeType(n).upper() == "TANK":
            node_obj = {
                "id": "Tank" + str(obj_counts["Tank"]),
                "type": "Tank",
                "contents": content_placeholder,
                "levation (meters)": G.getNodeElevations(n),
            ...
            node_ids[n] = "Tank" + str(obj_counts["Tank"])
            nodes["Tank" + str(obj_counts["Tank"])] = node_obj
            obj_counts["Tank"] += 1
...
if __name__ == "__main__":
    epyt2pypes(
        "json/others/EPANET Net 3.inp", "json/others/EPANET Net 3.json", name="EPANET Net 3"
    )
