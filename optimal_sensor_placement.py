...
parser.add_argument(
    "--mode", "-m", type=int, default=1, help="Which WWTP to use (1, 2, 3)"
)
...
parser.add_argument(
    "--original_log_file", "-o", type=str, help="Path to original log file"
)
...
class SensorOptimizer:
    def __init__(
        self, cost_weights, observability_weights, redundancy_weights, json_path=None
    ):
        # Create graph
    ...
    def __repr__(self):
        return (
            f"SensorOptimizer with {len(self.nodes)} nodes and {len(self.edges)} edges"
        )
    ...
    def redundancy_function(self, x, verbose=False):
        ...
        if (
            upper_bounds_A[0] < lower_bounds_B[0]
            and upper_bounds_A[1] < lower_bounds_B[1]
            and upper_bounds_A[2] < lower_bounds_B[2]
        ):
            return True
        return False
            ...
            for j, other in enumerate(layouts):
                if i != j:
                    ...
                    if (np.array(other[1:]) <= np.array(current[1:])).all() and (
                        np.array(other[1:]) < np.array(current[1:])
                    ).any():
                        is_dominated = True
                        break
        ...
        if path:
            with open(path, "w") as f:
                ...
                for layout, cost, obs, red in self.pareto_front:
                    f.write(
                        f"({cost:.0f}, {self.n-obs:.0f}, {self.n-red:.0f})\t{layout}\n"
                    )
    ...
    def visualize_pareto_solutions(self, save_path=None):
        pareto_solutions = [
            (layout[1], layout[2], layout[3]) for layout in self.pareto_front
        ]
        ...
        for i in range(len(costs)):
            ax.plot(
                [costs[i], costs[i]],
                [observabilities[i], observabilities[i]],
                [0, redundancies[i]],
                c="gray",
                linestyle="--",
                linewidth=0.8,
...
def plot_pareto_3d(file_path, plant_no=1, margin=0.73):
    ...
    Sizes = counts**0.5
    Sizes = (
        0.27 + 0.73 * (Sizes - np.min(Sizes)) / (np.max(Sizes) - np.min(Sizes))
    ) * 23
    print(f"Sizes:\n{Sizes}")
    ...
    for i in range(len(unique_points)):
        ax.plot3D(
            [MaxScale + margin, unique_points[i, 0]],
        ...
        ax.plot3D(
            [unique_points[i, 0], unique_points[i, 0]],
    ...
    ax.plot3D(
        [0, 0], [-margin, nO], [nR, nR], "--", color=[0.42] * 3, linewidth=0.5
    )  # y-plane projection at x=nC
    ax.plot3D(
        [0, 0], [nO, nO], [-margin, nR + margin], "--", color=[0.42] * 3, linewidth=0.5
    )  # z-plane projection at x=nC
    ax.plot3D(
        [-margin, nC], [nO, nO], [nR, nR], "--", color=[0.42] * 3, linewidth=0.5
    )  # x-plane projection ending at x=nC
    ...
    conditions = [
        (
            np.logical_and(
                np.logical_and(unique_points[:, 1] != nO, unique_points[:, 0] != 0),
                np.logical_and(unique_points[:, 2] != nR, unique_points[:, 2] != 0),
            ),
            green,
        ),  # Green
        (
            np.logical_and(
                unique_points[:, 1] == nO,
                unique_points[:, 0] + np.sum(Layouts[0]) == unique_points[:, 2],
            )[1:],
            red_color,
        ),  # Red
        (
        ...
        (
            np.logical_and(unique_points[:, 1] == nO, unique_points[:, 2] == 0),
            blue,
        ),  # Blue
        (unique_points[:, 0] == 0, "k"),  # Black
        (
            np.logical_and(
                np.logical_and(unique_points[:, 0] != 0, unique_points[:, 2] == 0),
                np.logical_and(unique_points[:, 1] != 0, unique_points[:, 1] != nO),
            ),
            "w",
        ),  # White
    ]
...
def run():
    ...
    optimizer.visualize_pareto_solutions(
        save_path="results/opt_sensor/pareto_front_WWTP2.png"
    )
