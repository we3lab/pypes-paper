...
parser.add_argument(
    "--tag_file",
    type=str,
    default="data/desalination/tag_list.txt",
    help="Path to the tag file",
)
parser.add_argument(
    "--data_folder",
    type=str,
    default="data/desalination",
...
parser.add_argument(
    "--data_plot",
    type=str,
    default="results/fault_detection/data.png",
    help="Path to save the data plot",
)
parser.add_argument(
    "--json_path",
    type=str,
    default="json/desalination.json",
    help="Path to the network JSON file",
)
parser.add_argument(
    "--T2Q_plot",
    type=str,
    default="results/fault_detection/T2Q_plot.png",
    help="Path to save the T2Q plot",
)
parser.add_argument(
    "--PC_plot",
    type=str,
    default="results/fault_detection/PC_plot.png",
    help="Path to save the PC plot",
)
...
class MeasurementData:
    ...
    def filter_data(self, data, mode=1, col_name="Permeate flowrate (GPM)", cutoff=500):
        ...
        if mode == 0:
            ...
            print(
                f"Filtered data with {col_name} <= {cutoff}, new shape: {data_cp.shape}"
            )
        elif mode == 1:
            data_cp = data_cp[data_cp[col_name] >= cutoff]
            print(
    ...
    def detect_and_clean_outliers(self, time_series, delta=0.3, K=1, bandwidth=13):
        ...
        forecast = pd.Series(
            np.zeros_like(time_series), index=time_series.index
        )  # Forecast values
        sigma_e = pd.Series(
            np.zeros_like(time_series), index=time_series.index
        )  # Forecast standard deviation
        accepted_data = pd.Series(
            np.zeros_like(time_series), index=time_series.index
        )  # Cleaned data
        forecast.iloc[0] = time_series.iloc[0]  # Initialize first forecast

        delta_t = (
            abs(time_series.iloc[1] - forecast.iloc[0]) if len(time_series) > 1 else 0
        )

        for t in range(1, len(time_series)):
            forecast.iloc[t] = (1 - delta) * forecast.iloc[
                t - 1
            ] + delta * time_series.iloc[t - 1]
    ...
    def preprocess_data(
        ...
        # Filter data
        if filter_data != -1:
            self.training_data_filtered = self.filter_data(
                self.training_data, filter_data
            )
            self.testing_data_filtered = self.filter_data(
                self.testing_data, filter_data
            )
        ...
        if remove_outliers == 1:
            for col in self.training_data_filtered.columns:
                ...
                IQR = Q3 - Q1
                self.training_data_filtered = self.training_data_filtered[
                    (self.training_data_filtered[col] >= Q1 - 1.5 * IQR)
                    & (self.training_data_filtered[col] <= Q3 + 1.5 * IQR)
                ]
        elif remove_outliers == 0:
            for col in self.training_data_filtered.columns:
                if col == "timestamp":
                    continue
                self.training_data_filtered[col] = self.detect_and_clean_outliers(
                    self.training_data_filtered[col]
                )
        ...
        print(
            f"Preprocessed data with shape: {processed_data.shape}, train-test split: {self.train_test_split}"
        )
        print(
            f"Filtered data with shape: {self.training_data_filtered.shape, self.testing_data_filtered.shape}"
        )
    ...
    def plot_data(self, save=None, mode="test", shade=True, combined=True):
        ...
        if combined:
            ...
            axs[0].plot(
                dates,
                plot_data["Permeate flowrate (GPM)"],
                label="Permeate",
                color=RO_item_to_color["Permeate"],
                zorder=2,
            )
            axs[0].plot(
                dates,
                plot_data["Brine flowrate (GPM)"],
                label="Brine",
                color=RO_item_to_color["Brine"],
                zorder=2,
            )
            if shade:
            ...
            axs[1].plot(
                dates,
                plot_data["Permeate conductivity (uS/cm)"],
                label="Permeate",
                color=RO_item_to_color["Permeate"],
                zorder=1,
            )
            if shade:
                for i, (start, end) in enumerate(off_hours):
                    axs[1].axvspan(
                        dates[start], dates[end - 1], color=gray_color, alpha=0.3
                    )
            axs[1].set_ylabel("Conductivity\n(uS/cm)", fontsize=y_label_fontsize)
            axs[1].set_ylim(
                bottom=0, top=plot_data["Permeate conductivity (uS/cm)"].max() * 1.1
            )

            # HP pump speed (Hz) and Circulation pump speed (Hz)
            axs[2].plot(
                dates,
                plot_data["HP pump speed (Hz)"],
                label="HP pump",
                color=RO_item_to_color["HP pump"],
                linestyle="--",
            )
            axs[2].plot(
                dates,
                plot_data["Circulation pump speed (Hz)"],
                label="Circulation pump",
                color=RO_item_to_color["Circulation pump"],
                linestyle="--",
            )
            if shade:
                for i, (start, end) in enumerate(off_hours):
            ...
            axs[3].plot(
                dates,
                plot_data["HP pump pressure (PSI)"],
                label="HP pump",
                color=RO_item_to_color["HP pump"],
                linestyle="--",
            )
            axs[3].plot(
                dates,
                plot_data["Circulation pump pressure (PSI)"],
                label="Circulation pump",
                color=RO_item_to_color["Circulation pump"],
                linestyle="--",
            )
            ...
            axs[3].set_ylim(
                bottom=0, top=plot_data["HP pump pressure (PSI)"].max() * 1.1
            )
...
class FaultDetectionSystem:
    ...
    def pca_scree_plot(self, data):
        ...
        plt.plot(
            np.arange(1, len(explained_variance) + 1), explained_variance, marker="o"
        )
    ...
    def fit_pca_model(self):
        ...
        self.T2_threshold = (a * (m - 1) / (m - a)) * f.ppf(
            1 - self.significance_level, dfn=a, dfd=m - a
        )
        self.T2_threshold_1 = (a * (m - 1) / (m - a)) * f.ppf(
            1 - 0.01, dfn=a, dfd=m - a
        )
        self.T2_threshold_5 = (a * (m - 1) / (m - a)) * f.ppf(
            1 - 0.05, dfn=a, dfd=m - a
        )
        ...
        self.Q_threshold = residual_var * f.ppf(
            1 - self.significance_level, dfn=1, dfd=(m - a)
        )
    ...
    def plot_pc(self, save=None):
        ...
        scaling_factor = 5
        ax1.scatter(
            self.scores[:, 0] / scaling_factor,
            self.scores[:, 1] / scaling_factor,
            c="yellow",
            alpha=0.3,
            s=2,
        )
        ...
        for i, (pc1, pc2) in enumerate(
            zip(self.pca.components_[0], self.pca.components_[1])
        ):
            ax1.arrow(
                0,
                0,
                pc1,
                pc2,
                color=RO_name_to_color[self.dataset.training_data.columns[i + 1]],
                alpha=0.5,
            )
            ax1.text(
                pc1,
                pc2,
                self.dataset.training_data.columns[i + 1],
                color=RO_name_to_color[self.dataset.training_data.columns[i + 1]],
                fontsize=12,
            )
    ...
    def detect_faults(self, virtual_tags=False):
        ...
        T2_stats = np.sum(
            (scores_new / np.sqrt(self.pca.explained_variance_)) ** 2, axis=1
        )
        ...
        return pd.DataFrame(
            {
                "T2": T2_stats_full_length,
                "Q": Q_stats_full_length,
    ...
    def plot_fault_detection(self, stats_df, save=None):
        ...
        for i, (start, end) in enumerate(self.dataset.off_hours):
            axs[0].axvspan(
                dates[start], dates[end - 1], color=gray_color, alpha=0.3, zorder=2
            )
        axs[0].set_ylim(0, 25)
        axs[0].axhline(y=self.T2_threshold_1, color="#fb9a99", label="α=0.01")
        axs[0].axhline(
            y=self.T2_threshold_5, color="#fb9a99", linestyle="--", label="α=0.05"
        )
        ...
        axs[1].axhline(
            y=self.Q_threshold_5, color="#fb9a99", linestyle="--", label="α=0.05"
        )
        ...
        axs[1].xaxis.set_major_locator(
            DayLocator(interval=2)
        )  # Set major ticks to daily intervals
        axs[1].xaxis.set_major_formatter(
            DateFormatter("%b %d")
        )  # Format as "Month Day" (e.g., "Feb 20")
    ...
    def construct_virtual_tags(self):
        ...
        for pc_index in range(min(2, self.n_components)):
            ...
            operations = (
                "lambda "
                + ", ".join("x_{} ".format(i + 1) for i in range(len(tags)))
                + ": "
                + " + ".join(
                    [
                        f"({coef:.5f} * x_{i+1})"
                        for i, (coef, tag) in enumerate(zip(pc_coeffs, tags))
                        if abs(coef) > 1e-5
                    ]
                )
            )
    ...
    def smooth_with_drop_preservation(data, window_size=5, polyorder=2):
        ...
        drops = np.where(
            np.diff(data) < -5, True, False
        )  # Adjust -5 threshold based on your data's drop magnitude
        drop_indices = (
            np.where(drops)[0] + 1
        )  # Shift indices because diff reduces length by 1
    ...
    def plot_combined_data_and_faults(self, plot_data, stats_df, save=None, shade=True):
        ...
        if shade:
            for i, (start, end) in enumerate(off_hours):
                axs[0].axvspan(
                    dates[start], dates[end - 1], color=gray_color, alpha=0.3, zorder=1
                )
        axs[0].set_ylabel("Flowrate\n(GPM)", fontsize=y_label_fontsize)
        ...
        smoothed_hp_pressure = self.smooth_with_drop_preservation(
            hp_pressure, window_size=20, polyorder=2
        )
        smoothed_circ_pressure = self.smooth_with_drop_preservation(
            circ_pressure, window_size=5, polyorder=2
        )
        axs[3].plot(
            dates,
            smoothed_hp_pressure,
            label="HP pump",
            color=RO_item_to_color["HP pump"],
            linestyle="--",
        )
        ...
        axs[3].set_ylabel("Pump\npressure\n(PSI)", fontsize=y_label_fontsize)
        axs[3].set_ylim(bottom=0, top=1000)
        ...
        axs[5].xaxis.set_major_locator(
            DayLocator(bymonthday=set(day_display))
        )  # Set major ticks to daily intervals
...
if __name__ == "__main__":
    ...
    fd_system.plot_combined_data_and_faults(
        dataset.testing_data,
        stats_df,
        save="results/fault_detection/all.png",
        shade=True,
    )
